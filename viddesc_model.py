...
class VideoDesc_Model(Model_Wrapper):
    ...
    def ArcticVideoCaptionNoLSTMEncWithInit(self, params):
        ...
        if params['BEAM_SEARCH']:
            ...
            shared_additional_output_merge = eval(params['ADDITIONAL_OUTPUT_MERGE_MODE'])(name='additional_input_model_next')
    ...
    def LinkedTranslation(self, params):
        """
...
#--->
        ...
        if params['WEIGHTED_MERGE']:
            ...
            additional_output = shared_additional_output_merge([out_layer_mlp, out_layer_ctx, out_layer_emb, out_layer_prev])
    ...
    # Bypass
    def LinkedTranslation4(self, params):
            """
            Conditional Translation with:
                * Attention mechanism
                * Conditional LSTM
            ...
            if len(params['INIT_LAYERS']) > 0:
                ...
                initial_state = Regularize(initial_state, params, name='initial_state')
                input_attentional_decoder = [emb, annotations, initial_state]
                ...
                input_attentional_decoder = [emb, annotations]
            ...
            #out_layer_prev = shared_Lambda_Permute(out_layer_prev)
            # Multiply by zero the previous encoder attention vector
            out_layer_prev = Lambda(lambda x: K.dot(x, 0), name='multiply_by_zero')(out_layer_prev)
    ...
    # Initialize Decoder with last prev_decoder_state
    def LinkedTranslation5(self, params):
            """
            Conditional Translation with:
                * Attention mechanism
                * Conditional LSTM
            ...
            if len(params['INIT_LAYERS']) > 0:
                ...
                # Cogemos el ultimo estado del encoder previo y se lo enchufamos como estado inicial al decoder
                ctx_mean = prev_desc_enc
                for n_layer_init in range(len(params['INIT_LAYERS']) - 1):
                    ctx_mean = Dense(params['DECODER_HIDDEN_SIZE'], name='init_layer_%d' % n_layer_init,
                                 kernel_initializer=params['INIT_FUNCTION'],
                                 kernel_regularizer=l2(params['WEIGHT_DECAY']),
    ...
    def DoubleLinkedTranslation(self, params):
        """
        Video captioning with:
            * Attention mechanism on video frames
            * Conditional LSTM for processing the video
